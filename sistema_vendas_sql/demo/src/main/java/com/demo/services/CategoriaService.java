...
public class CategoriaService {
    ...
    public CategoriaResponseDTO findById(Long id) {
        return new CategoriaResponseDTO(categoriaRepository.findById(id).orElseThrow(() -> new NotFoundException("Categoria não encontrada")));
    }

    public CategoriaResponseDTO findByNome(String nome) {
        if(!categoriaRepository.existsByNome(nome)) throw new NotFoundException("Não existe categoria com este nome");
    ...
    public CategoriaResponseDTO createCategoria(CategoriaRequestDTO data){
        if(categoriaRepository.existsByNome(data.nome())) throw new AlreadyExistsException("Já existe categoria com este nome");
    ...
    public CategoriaResponseDTO updateCategoria(Long id, CategoriaRequestDTO data){
        Categoria updatedCategoria = categoriaRepository.findById(id).orElseThrow(() -> new NotFoundException("Categoria inexistente"));
        if(categoriaRepository.existsByNome(data.nome()) && data.nome().equals(updatedCategoria.getNome())) throw new AlreadyExistsException("Já existe uma categoria com este nome");
    ...
    public void deleteCategoria(Long id){
        Categoria categoria = categoriaRepository.findById(id).orElseThrow(() -> new NotFoundException("Categoria inexistente"));
